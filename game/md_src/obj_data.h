 ...
 * to show this tick, and its flip bits (obj_emit_pieces' own flipH/flipV).
 * frame == OBJ_SKIP means "do not draw this entry at all this frame"
 * (off-screen in Y, already collected, the type not yet triggered, ...). */
typedef struct {
	uint16_t frame;
	uint8_t  flipH, flipV;
} ObjDrawDecision;
