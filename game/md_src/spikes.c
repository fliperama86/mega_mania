...
static uint8_t spikeShown[SPIKES_COUNT];
...
static void spikes_tick_advance(void)
{
    ...
    for (i = 0; i < SPIKES_COUNT; i++) {
        ...
        /* Spikes.c:20-27 (HIDDEN->APPEAR) and :41-47 (SHOWN->DISAPPEAR)
         * both gate on the identical `stagger<<6 == (timer&0x40)` (only
         * evaluated on the HIDDEN side in the original, but see this file's
         * own header comment for why the combined condition below recurs
         * once every 128 ticks regardless of which state currently holds --
         * that is exactly the real transition period, just applied
         * symmetrically to both toggle directions here). */
...
void spikes_init(void)
{
    ...
    for (i = 0; i < SPIKES_COUNT; i++) {
        ...
        /* Spikes_Create:380-384: a moving spike starts fully retracted. */
                     ...
                     int16_t sonicWorldX, int16_t sonicWorldY, uint16_t sonicFrameIndex)
{
    ...
    for (i = lo; i < hi && n < SPIKES_SPRITE_CAP; i++) {
        ...
        if (spikeShown[i] != SPIKE_SHOWN) continue;
